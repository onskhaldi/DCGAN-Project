...
def evaluate(D, G, E, G_latent_dim, test_loader):
    D.eval()
    G.eval()
    E.eval()
    ...
    with torch.no_grad():
        for batch_imgs, _ in test_loader:
            ...
            D_real_output = D(batch_imgs, E(batch_imgs))
            ...
            z = torch.randn(batch_imgs.size(0), G_latent_dim, 1, 1, device=device)
            fake_imgs = G(z)
            D_output_on_fake = D(fake_imgs, z)
            G_loss = nn.functional.binary_cross_entropy_with_logits(D_output_on_fake, real_labels)
            G_batch_losses.append(G_loss.item())

            # Encoder loss
            E_loss = nn.functional.binary_cross_entropy_with_logits(
                D_real_output, fake_labels
            )
            E_batch_losses.append(E_loss.item())

    D.train()
    G.train()
    E.train()

    return np.mean(D_batch_losses), np.mean(G_batch_losses), np.mean(E_batch_losses)
